...
- **GPU compute (`cartan-gpu`)**: portable GPU primitives built on `wgpu` 29 (Vulkan) with optional CUDA via `cudarc` 0.19. Unified `Fft` trait implemented by both `VkFftBackend` (VkFFT-backed) and `CuFftBackend` (cuFFT-backed) with identical `forward∘inverse = identity` semantics. Runtime backend selection via `UniFftBackend` + `UniBuffer`. **Zero-copy interop**: `SharedFftBuffer` exports `VkDeviceMemory` via `VK_KHR_external_memory_fd` and imports it into CUDA via `cuImportExternalMemory`, letting VkFFT and cuFFT operate on the same physical bytes — proven end-to-end (Vk Forward → CUDA Inverse round-trip L-inf = 9e-7 on RTX 5060 Laptop, Blackwell SM 12.0, CUDA 13.1). Linux-only for the shared-memory path; the per-backend FFT paths are cross-platform.
