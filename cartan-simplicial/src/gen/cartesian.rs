...
use cartan_exterior::combo::factorial;
...
pub fn linear_index2cartesian_index(
    ...
    dim_len: usize,
    dim: usize,
) -> nalgebra::DVector<usize> {
    let mut cart_idx = nalgebra::DVector::zeros(dim);
    for icomp in 0..dim {
...
pub fn cartesian_index2linear_index(cart_idx: nalgebra::DVector<usize>, dim_len: usize) -> usize {
...
impl CartesianMeshInfo {
    ...
    pub fn ncells(&self) -> usize {
        (self.ncells_axis.pow(self.dim() as u32)) * (factorial(self.dim()) as usize)
    }
    pub fn nvertices(&self) -> usize {
        self.nvertices_axis().pow(self.dim() as u32)
    }
    pub fn vertex_cart_idx(&self, ivertex: usize) -> nalgebra::DVector<usize> {
    ...
    pub fn boundary_vertices(&self) -> Vec<usize> {
        ...
        for d in 0..self.dim() {
            ...
            for ivertex in 0..nvertices_boundary_facet {
                let vertex_icart = linear_index2cartesian_index(ivertex, self.nvertices_axis(), self.dim() - 1);
                ...
                r.insert(low_boundary);
                r.insert(high_boundary);
            }
        }
        r.into_iter().collect()
    }
...
impl CartesianMeshInfo {
    ...
    pub fn compute_vertex_coords(&self) -> MeshCoords {
        let mut coords = nalgebra::DMatrix::zeros(self.dim(), self.nvertices());
    ...
    pub fn compute_cell_skeleton(&self) -> Skeleton {
        ...
        let dim = self.dim();
        let nsimplices = (factorial(dim) as usize) * nboxes;
...
mod cartan_tests {
    ...
    #[test]
    fn unit_square_2x2_counts() {
        // 2x2 grid: 3x3 = 9 vertices, 2*2*2 = 8 triangles (2 per box).
        let info = CartesianMeshInfo::new_unit(2, 2);
        assert_eq!(info.nvertices(), 9);
        assert_eq!(info.ncells(), 8);
        let (complex, coords) = info.compute_coord_complex();
        assert_eq!(complex.dim(), 2);
        assert_eq!(coords.nvertices(), 9);
        // boundary of the unit square: all 8 perimeter vertices.
        assert_eq!(info.boundary_vertices().len(), 8);
    }

    #[test]
    fn unit_cube_yields_realizable_edge_lengths() {
        let info = CartesianMeshInfo::new_unit(3, 2);
        let (complex, coords) = info.compute_coord_complex();
        assert_eq!(complex.dim(), 3);
