...
impl SlabProblem {
    ...
    pub fn solve(&self) -> Result<SlabSolution, HomogError> {
        ...
        let vertices: Vec<Vector3<f64>> = unit_mesh
            .vertices
            .iter()
            .map(|v| Vector3::new(v.x * self.l_x, v.y * self.l_y, v.z * self.h))
            .collect();
        ...
        let barycenters: Vec<Vector3<f64>> = mesh
            .simplices
            .iter()
            .map(|tet| {
                (mesh.vertices[tet[0]]
            ...
            .collect();
        let k_per_tet: Vec<Matrix3<f64>> = barycenters.iter().map(|b| (self.k_of_z)(b.z)).collect();
        ...
        for s in 0..nt {
            ...
            let jit = jac
                .try_inverse()
                .ok_or(HomogError::Mesh(alloc::format!("slab tet {s} singular")))?
                .transpose();
        ...
        for (i, v) in mesh.vertices.iter().enumerate() {
            if v.z < 1e-9 {
                prescribed.push((i, self.p_bot));
            } else if v.z > self.h - 1e-9 {
                prescribed.push((i, self.p_top));
            }
        }
        ...
        for s in 0..nt {
            let tet = mesh.simplices[s];
            let g = &grads[s];
            let grad_p = p[tet[0]] * g[0] + p[tet[1]] * g[1] + p[tet[2]] * g[2] + p[tet[3]] * g[3];
...
fn apply_dirichlet_values(
    ...
    let prescribed_map: HashMap<usize, f64> = prescribed.iter().copied().collect();
    let n = a.nrows();
    ...
    for (i, j, &val) in a.triplet_iter() {
        if prescribed_map.contains_key(&i) {
            continue;
        }
