...
fn maxwell_style<O: TensorOrder>(
    rve: &Rve<O>,
    opts: &SchemeOpts,
) -> Result<Effective<O>, HomogError>
where
    Sphere: Shape<O>,
{
    ...
    for ph in &rve.phases {
        ...
        let a = ph
            .shape
            .concentration_dilute(&c_ref, &ph.property, &opts.integration)?;
