...
impl WishartRveEnsemble {
    pub fn new(
        base_rve: Rve<Order2>,
        perturbed_phase: impl Into<String>,
        mean: nalgebra::Matrix3<f64>,
...
pub fn karcher_mean_spd3(
    xs: &[nalgebra::Matrix3<f64>],
    max_iter: usize,
    tol: f64,
) -> Result<nalgebra::Matrix3<f64>, HomogError> {
    if xs.is_empty() {
        return Err(HomogError::Solver(String::from(
            "karcher_mean_spd3: empty input",
        )));
    }
    let spd = Spd::<3>;
    let mut mean = xs[0];
...
mod tests {
    ...
    fn karcher_mean_of_identical_points_is_point() {
        ...
        assert!(
            d < 1e-10,
            "Karcher mean of identical points should be that point, got d={d}"
        );
    }
    ...
    fn ensemble_sampling_runs_end_to_end() {
        ...
        rve.add_phase(Phase {
            name: String::from("I"),
            shape: Arc::new(Sphere),
            property: mean_inclusion,
            fraction: 0.2,
        });
        rve.set_matrix("M");

        let mut ensemble = WishartRveEnsemble::new(rve, "I", mean_inclusion, 10.0, 10);
        ensemble.burn_in = 10;
        ensemble.dt = 1e-4;

        let mut rng = SmallRng::seed_from_u64(42);
        let result = ensemble
            .sample(&MoriTanaka, &SchemeOpts::default(), &mut rng)
            .unwrap();
        assert!(result.variance >= 0.0);
        // Karcher mean must be SPD: positive eigenvalues.
        let eig = result.frechet_mean.symmetric_eigen();
        assert!(
            eig.eigenvalues.iter().all(|v| *v > 0.0),
            "Karcher mean not SPD: eigenvalues = {:?}",
            eig.eigenvalues
        );
    }
}
