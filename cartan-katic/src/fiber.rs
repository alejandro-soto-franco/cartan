//! The k-atic fibre: a rotor with amplitudes.
//!
...
impl<A: Default> Default for KaticElement<A> {
    fn default() -> Self {
        Self {
...
impl<A: AsRef<[f64]>> KaticElement<A> {
...
/// The fibre of k-atic states for symmetry `H`.
#[derive(Clone, Copy, Debug, Default)]
pub struct KaticFiber<H: SymmetryGroup>(core::marker::PhantomData<H>);
...
impl<H: SymmetryGroup> Fiber for KaticFiber<H> {
    ...
    fn transport_by(rotation: &[f64], d: usize, element: &Self::Element) -> Self::Element {
        debug_assert_eq!(d, 3, "KaticFiber requires d=3");
    ...
    fn transport_by_rotor(rotor: &Rotor, element: &Self::Element) -> Self::Element {
        match rotor {
            ...
            Rotor::R2(_) => {
                debug_assert!(false, "KaticFiber requires a 3D rotor");
                *element
            }
        }
    }
...
impl<H: SymmetryGroup> FiberOps for KaticFiber<H> {
    fn accumulate_diff(
...
mod tests {
    ...
    fn rotor_and_matrix_transport_agree() {
        ...
        let via_rotor = <KaticFiber<AxialApolar>>::transport_by_rotor(&Rotor::R3(rot), &elem);
        let via_matrix = <KaticFiber<AxialApolar>>::transport_by(&rot.to_matrix(), 3, &elem);
        for (a, b) in [
    ...
    fn transport_preserves_the_unit_sphere() {
        ...
        let moved = <KaticFiber<AxialApolar>>::transport_by_rotor(&Rotor::R3(rot), &elem);
