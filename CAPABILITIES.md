...
Detailed per-crate inventory. The [README](README.md) is the on-ramp; this file
is the reference for what each crate contains.
...
- **Homogenisation**: `cartan-homog` implements the full microscopic-to-macroscopic pipeline from the [RVE-to-mesh blog post](https://sotofranco.dev/blog/posts/rve-to-mesh-pipeline): 10 mean-field schemes (Voigt, Reuss, Dilute, DiluteStress, Mori-Tanaka, Self-Consistent with SPD-geodesic iteration, Asymmetric SC, Maxwell, PCW, Differential + compliance dual), a full-field DEC cell-problem solver with periodic BCs, 4-tier solver ladder (Jacobi, ILU(0), AMG, dense LU), Lebedev quadrature for anisotropic references, voxel import for micro-CT data, and Wishart stochastic ensembles. Cross-validated against an independent reference implementation to d_AI < 3e-15 on 84 cases spanning 4 shapes, 10 schemes, and 2 tensor orders. 200x median speedup over the reference per call. All interaction-corrected schemes respect Hashin-Shtrikman bounds.
...
- **GPU compute (`cartan-gpu`)**: wgpu 29 compute primitives (`Device`, `GpuBuffer<T>`, `Kernel`). FFT is delegated to the standalone [`gpufft`](https://crates.io/crates/gpufft) crate (v0.1), which provides a backend-agnostic trait surface with Vulkan (VkFFT) and CUDA (cuFFT) backends. Enable the `fft` feature on `cartan-gpu` to get `cartan_gpu::gpufft` as a re-export. **Zero-copy interop** (Linux, `gpufft::shared`): `SharedFftBuffer` exports `VkDeviceMemory` via `VK_KHR_external_memory_fd` and imports it into CUDA via `cuImportExternalMemory`, letting both backends operate on the same physical bytes. Proven end-to-end: Vk Forward → CUDA Inverse round-trip L-inf = 9e-7 on RTX 5060 Laptop, Blackwell SM 12.0, CUDA 13.1.
...
cartan runs on embedded and no_std targets. The geometry core (manifolds, geodesics, optimisation) compiles without the standard library, as do the mean-field homogenisation schemes. The discrete exterior calculus layer (`cartan-dec`) requires std: it operates on heap-allocated mesh structures and is not designed for embedded use. CI builds the no_std configuration for `thumbv7em-none-eabihf` on every run.
...
| `cartan-dec` | entire crate (mesh allocation) |
