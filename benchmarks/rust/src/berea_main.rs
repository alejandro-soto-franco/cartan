...
use cartan_homog::{
    rve::{Phase, Rve},
    schemes::{
        Differential, Dilute, DiluteStress, Maxwell, MoriTanaka, ReussBound, Scheme, SchemeOpts,
        SelfConsistent, VoigtBound,
    },
    shapes::Sphere,
    tensor::Order4,
};
...
fn time_and_extract<S: Scheme<Order4>>(
    scheme: &S,
    rve: &Rve<Order4>,
    opts: &SchemeOpts,
) -> (f64, f64, u128) {
    ...
    for _ in 0..WARMUP {
        let _ = scheme.homogenize(rve, opts);
    }
    let mut last_k = 0.0;
    let mut last_g = 0.0;
    for _ in 0..REPS {
...
fn main() {
    let out_path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "benchmarks/results/berea_cartan.jsonl".into());
    ...
    let schemes: &[(&str, BenchFn)] = &[
        ...
        ("DIFF", &|| {
            time_and_extract(&Differential::default(), &rve, &opts)
        }),
    ];
